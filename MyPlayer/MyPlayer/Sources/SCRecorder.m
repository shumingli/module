...
- (UIImage *)_imageFromSampleBufferHolder:(SCSampleBufferHolder *)sampleBufferHolder {
    ...
    UIImage *imaone=[ UIImage imageWithCIImage:ciImage];
    
    
    UIImage *image = [UIImage imageWithCGImage:cgImage];
       ;
    UIImage *resuIamge=[UIImage imageWithCIImage:ciImage];
    
    UIGraphicsBeginImageContext(CGSizeMake(640,resuIamge.size.height/resuIamge.size.width*640 ));
    [resuIamge drawInRect:CGRectMake(0, 0, 640, resuIamge.size.height/resuIamge.size.width*640)];
    UIImage *newImage=UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
    
    NSLog(@"%f         %f",image.size.width,image.size.height);
    NSLog(@"%f         %f",resuIamge.size.width,resuIamge.size.height);
    NSLog(@"%f         %f",newImage.size.width,newImage.size.height);
    NSLog(@"%f         %f",imaone.size.width,imaone.size.height);

    
    CGImageRelease(cgImage);
    CFRelease(sampleBuffer);
//    self.photoOutputSettings=[NSDictionary dictionaryWithObjects:@[image] forKeys:@[@"img"]];
